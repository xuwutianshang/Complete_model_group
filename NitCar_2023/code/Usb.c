...
{
    uart_write_byte_wait(USB_UART_INDEX, data);
    printf("TX: %x\r\n",data);
}
...
{	 
    uint8_t Uart1Res;
    uart_query_byte(USB_UART_INDEX, &Uart1Res);
    printf("RX: %x\r\n", Uart1Res);
    if(Uart1Res<=180 && Uart1Res>=0 )
    {
        //printf("PWM :%d\r\n",(uint32)SERVO_MOTOR_DUTY(Uart1Res));
        //pwm_set_duty(SERVO_MOTOR_PWM, (uint32)SERVO_MOTOR_DUTY(Uart1Res));
        //SERVO_AngleControl(0.0);
    }
